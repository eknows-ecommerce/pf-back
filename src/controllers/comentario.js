...
const create = async (req, res, next) => {
  ...
  try {
    if (!comment.texto)
      return res.status(400).json({ msg: 'Texto no provisto' })
    if (!comment.likes)
      return res.status(400).json({ msg: 'Likes no provisto' })
...
const updateById = async (req, res, next) => {
  ...
  try {
    ...
    if (!updatedComment)
      return res
        .status(200)
        .json({ msg: 'No se pudo actualizar el comentario' })
...
const createBulk = async (req, res, next) => {
  ...
  try {
    if (!comentarios)
      return res.status(400).json({ msg: 'Comentarios no provistos' })
    ...
    res
      .status(201)
      .json({ comentarios: newComentarios, msg: 'Comentarios creados' })
  } catch (error) {
    next(error)
  }
}
